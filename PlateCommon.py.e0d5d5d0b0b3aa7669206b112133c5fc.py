...
def rot(img, angel, shape, max_angel, bbox=False, padding=0):
    """ 使图像轻微的畸变
        img 输入图像
        factor 畸变的参数
        size 为图片的目标尺寸
    ...
    if bbox:
        old_points = np.float32([[pts1[0][0] + padding, pts1[0][1] + padding],
                                [pts1[1][0] + padding, pts1[1][1] - padding],
                                [pts1[2][0] - padding, pts1[2][1] + padding],
                                [pts1[3][0] - padding, pts1[3][1] - padding]])
        old_points = old_points[:, np.newaxis, :]

        bbox_points = cv2.perspectiveTransform(old_points, M)
        # for pt in bbox_points:
        #     x,y = pt[0]
        #     cv2.circle(dst,(x, y), 10, (255,0,0), -1)

        return (dst, bbox_points)

    return dst


def rotRandrom(img, factor, size, bbox=None):
    shape = size
    pts1 = np.float32([[0, 0],
                        [0, shape[0]],
                        [shape[1], 0],
    ...
    if bbox is not None:
        bbox_points = cv2.perspectiveTransform(bbox, M)
        # for pt in bbox_points:
        #     x,y = pt[0]
        #     cv2.circle(dst,(x, y), 10, (255,0,0), -1)
...
def scaleRandom(img, ratio, bbox=None):
    ratio = 0.25 if ratio == 0 else ratio
...
def random_background(img, data_set, size):
    index = r(len(data_set))
    env = cv2.imread(data_set[index])
    env = cv2.resize(env, size)

    return env

def combine_images(background, top):
    ...
    top_fg = cv2.bitwise_and(top,top,mask = mask)
    # Put logo in ROI and modify the main image
    dst = cv2.add(background_bg,top_fg)
...
def GenCh(f, val, scale):
    img = Image.new("RGB", (23*scale, 70*scale), (255, 255, 255))
    draw = ImageDraw.Draw(img)
    # draw.text((0, 2), val.decode('utf-8'), (0, 0, 0), font=f)
    draw.text((0, 15*scale), val, (0, 0, 0), font=f)
    A = np.array(img)
    return A
